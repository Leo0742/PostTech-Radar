...
- экспериментировал с **Qwen3 Embeddings**, sentence-transformers и классическими моделями поверх embeddings;
...
**Embeddings:** sentence-transformers и Qwen3 Embeddings. Для финальных экспериментов я использовал embeddings вместе с prototype/kNN логикой и TF-IDF specialist для сложных случаев.
...
Исходный датасет, локальная SQLite-база, обученные model bundles и артефакты с примерами обращений **не публикуются**. В открытом репозитории оставлены ключевые части кода приложения, ML pipeline, тесты и dependency-файлы.

Причина простая: проект делался на предоставленных данных Service Desk, поэтому я не хочу выкладывать ticket-level данные или производные файлы, в которых они могут встречаться.
